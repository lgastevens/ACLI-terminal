# This script pulls from HTTP/FTP a list of latest files making the distribution
...
sub getFile {
	...
	if (!defined $prot) {
		foreach my $url (@urls) {
			...
			if ( ($prot eq 'ftp' && ftpGet($url, $file, $savePath, 1)) || ($prot eq 'http' && httpGet($url, $file, $savePath, 1) ) ) {
				$server = $url;
				print "\n";
				last;
			}
