...
1 - Install all the required software, modules and dependencies (non trivial..)
    Refer to: Installing_ACLI_required_libraries.txt
...
- Updates: update script not currently supported with Linux. If a new ACLI version exists, simply expand new zip file ontop of existing ACLI directory.
...
~$ acli username:password@192.168.10.10

For an SSH connection the -l option must be used:

~$ acli -l username 192.168.10.10
...
~$ acli -r 10.134.10.10 192.168.10.10
...
~$ acli -r rwa:rwa@10.134.10.10 username:password@192.168.10.10

The above examples could also have been written as:

~$ acli -r 10.134.10.10 "telnet 192.168.10.10"
~$ acli -r rwa:rwa@10.134.10.10 "telnet username:password@192.168.10.10"
...
~$ acli -r rwa:rwa@10.134.10.10 "ssh 192.168.10.10 -l admin"
...
~$ acli -r rwa:rwa@10.134.10.10 "ssh 192.168.10.10 -l admin:admin"
...
~$ acli -r -l rwa:rwa 10.134.10.10 "ssh 192.168.10.10 -l admin:admin"
...
~$ aftp 10.134.169.91-92,81-84,171-172 get config.cfg
Connecting to hosts:
  1 - 10.134.169.91
  2 - 10.134.169.92
  3 - 10.134.169.81
...
~$ aftp 10.134.169.91-92,81-84,171-172 put config.cfg
Connecting to hosts:
  1 - 10.134.169.91
  2 - 10.134.169.92
  3 - 10.134.169.81
...
~$ acmd.pl -o -f myhosts -s myscript ssh output.log
================================================================================
...
~$ acmd -l rwa -p rwa -x excel.csv!Switch ssh "enable; config term; prompt $name; router isis; sys-name $name; spbm 1 nickname $nickname"
================================================================================
...
~$ acligui 192.168.10.10-15,22,24
...
~$ acligui -u rwa 192.168.10.10-15,22,24
...
~$ acligui -u rwa:rwa 192.168.10.10-15,22,24
...
~$ acligui -g -u rwa:rwa 192.168.10.10-15,22,24
